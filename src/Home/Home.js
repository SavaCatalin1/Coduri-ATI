        ...
        if (existingCodes) {
            if (!existingCodes.includes(uniqueCode)) {
                ...
                try {
                    ...
                    await addDoc(collection(db, "produse"), { Denumire: name, Cod: uniqueCode, Created: timestamp, Pret: Number(Number(pretAchizitie) + (Number(pretAchizitie) * 20 / 100)) }).then(() => fetch())
                } catch (e) {
